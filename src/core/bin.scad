...
function new_bin(
  ...
  assert(
    is_num(base_thickness) && base_thickness >= 0 && base_thickness <= BASE_HEIGHT
  )
  assert(
    thumbscrew == false || base_thickness == BASE_HEIGHT,
    "Thumscrews are not compatible with custom base_thickness."
  )
  assert(
    !include_lip || fill_height <= 0 || fill_height <= height_mm - STACKING_LIP_SUPPORT_HEIGHT,
    str("Maximum fill_height for this bin is", height_mm - STACKING_LIP_SUPPORT_HEIGHT)
  )
  assert(is_bool(only_edges))
  let (
    fill_height_calculated = include_lip ?
      height_mm - BASE_HEIGHT - STACKING_LIP_SUPPORT_HEIGHT
    : height_mm - BASE_HEIGHT
  ) let (
    fill_height_real = fill_height > 0 ? fill_height
...
module bin_render_wall(bin) {
  ...
  let (base_grid = bin[1]) let (
    wall_height = grid_get_total_dimensions(base_grid) - [0, 0, BASE_HEIGHT]
  )
...
module bin_subdivide(bin, subdivisions) {
  ...
  assert(
    is_valid_2d(subdivisions) && subdivisions.x >= 0 && subdivisions.y >= 0
  );
...
function bin_get_bases(bin) =
  ...
  let (base_grid = bin[1]) as_2d(grid_get_num_elements(base_grid));
...
function bin_get_grid_dimensions(bin) =
  ...
  let (base_grid = bin[1]) as_2d(grid_get_element_dimensions(base_grid));
...
function bin_get_bounding_box(bin) =
  ...
  let (base_grid = bin[1]) let (include_lip = bin[4]) grid_get_total_dimensions(base_grid) + [0, 0, include_lip ? stacking_lip_height() : 0];
...
function bin_get_infill_size_mm(bin) =
  ...
  let (base_grid = bin[1]) let (base_thickness = bin[2]) let (fill_height = bin[3]) //May be negative.
  let (
    fill_height_total = max(
      fill_height + BASE_HEIGHT - base_thickness, 0
    )
  ) let (size_2d = as_2d(grid_get_total_dimensions(base_grid))) concat(size_2d - 2 * [d_wall, d_wall], fill_height_total);
...
function bin_get_height_breakdown(bin) =
  ...
  let (base_grid = bin[1]) let (include_lip = bin[4]) let (bin_height = grid_get_total_dimensions(base_grid).z) let (infill_height = bin_get_infill_size_mm(bin).z) let (total_height = bin_height + (include_lip ? stacking_lip_height() : 0)) let (center_height = bin_height - BASE_HEIGHT) let (free_space = center_height - infill_height - (include_lip ? STACKING_LIP_SUPPORT_HEIGHT : 0))
  ...
  assert(
    heights[1][1] == heights[2][1] + heights[3][1],
    "BASE_HEIGHT Calculation Error!"
  )
  assert(
    heights[8][1] == heights[9][1] + heights[10][1],
    "Stacking Lip Height Calculation Error!"
  )
  assert(
    (
      heights[5][1] + heights[6][1] + (include_lip ? heights[7][1] : 0)
    ) - heights[4][1] < 0.01,
    str(
  ...
  assert(
    heights[0][1] == heights[1][1] + heights[4][1] + (include_lip ? heights[8][1] : 0),
    "Total Height Calculation Error!"
  )
  assert(
    heights[0][1] == bin_get_bounding_box(bin).z,
    "Total Height Calculation Error!"
  )
...
function _bin_get_infill_grid(bin, exact) =
  ...
  let (infill_size_mm = bin_get_infill_size_mm(bin)) let (base_perimeter = grid_get_perimeter(base_grid)) let (
    infill_element_dimensions = concat(
      bin_get_grid_dimensions(bin),
      infill_size_mm.z
    )
  ) let (
    equal_grid = grid_from_total(
      concat(bin_get_bases(bin), 1), infill_size_mm, true
    )
  ) let (
    exact_grid = grid_from_other(
      base_grid,
  ...
  let (infill_grid = exact ? exact_grid : equal_grid) infill_grid;
