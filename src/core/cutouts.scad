...
function cgs(size = [1, 1], height = 0) =
  ...
  let (element = grid_element_current()) let (grid = grid_element_get_grid(element)) let (scaled = grid_scale(grid, size)) assert(
    is_valid_3d(scaled),
    "grid_element_current() must have at least 3 dimensions."
  )
  let (divider_mm = [d_div / 2, d_div / 2, 0]) let (calculated = as_3d(scaled) - divider_mm) assert(
    calculated.x > 0 && calculated.y > 0 && (calculated.z > 0 || height > 0),
...
module compartment_cutter(size_mm, scoop_percent = 0, tab_width = 0, tab_angle = 90, center_top = true) {
  ...
  assert(
    is_num(tab_angle) || (is_undef(tab_angle) && tab_width == 0)
  );
...
module _half_rounded_square(size_mm) {
  assert(
    is_valid_3d(size_mm) && is_positive(size_mm)
  );
...
module cut_compartment_auto(size_mm, style_tab = 5, tab_top_left_only = false, scoop_percent = 0) {
  ...
  let (element = grid_element_current()) grid_element_is_last_row(element) && grid_element_is_first_col(element);
