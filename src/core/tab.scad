...
function normalize_to_box(box_size, angle) =
  assert(is_list(box_size) && len(box_size) == 2 && box_size.x > 0 && box_size.y > 0)
  assert(is_num(angle))
  let (na = normalize_angle(angle)) let (ba = atan2(box_size.x, box_size.y)) let (is_sides = abs(na) < 45 || abs(na) > 135) let (corner_consts = [0, 90 - ba, 90, 90 + ba, 180]) sign(na) * corner_consts[floor(abs(na) / 45)] + (na % 45) * (is_sides ? (90 - ba) : ba) / 45;
...
let (element = grid_element_current()) grid_element_is_first_col(element) ? 134 // Left
...
function get_tab_angle(style_tab) =
  ...
  let (lookup = [90, _auto_tab_angle, 134, 90, 46, undef]) is_function(lookup[style_tab]) ? lookup[style_tab]()
  : lookup[style_tab];
