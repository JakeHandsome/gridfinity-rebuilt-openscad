...
module render_wall(size) {
  assert(
    is_valid_3d(size) && size.x > 0 && size.y > 0 && size.z >= 0
  );
...
function stacking_lip_height() =
  let (filleted_line = radius_line_edge(STACKING_LIP, 3, STACKING_LIP_FILLET_RADIUS)) max([for (l = filleted_line) l.y]);
...
function _index_to_points(line, index) =
  ...
  let (last_index = len(line) - 1) assert(
    is_num(index) && index >= 0 && index <= len(line) - 1
  )
  let (
    first_point = line[0] != line[last_index] ? line[0]
    : line[1]
  ) let (
    last_point = line[0] != line[last_index] ? line[last_index]
    : line[last_index - 1]
  ) [
...
module _test_index_to_points() {
  ...
  assert(
    _index_to_points(square_line, 0) == [
      [0, 2],
      [0, 0],
      [2, 0],
    ]
  ...
  assert(
    _index_to_points(square_line, 1) == [
      [0, 0],
      [2, 0],
      [2, 2],
    ]
  ...
  assert(
    _index_to_points(square_line, 2) == [
      [2, 0],
      [2, 2],
      [0, 2],
    ]
  ...
  assert(
    _index_to_points(square_line, 3) == [
      [2, 2],
      [0, 2],
      [0, 0],
    ]
  ...
  assert(
    _index_to_points(square_line, 4) == [
      [0, 2],
      [0, 0],
      [2, 0],
    ]
...
function point_angle(line, index) =
  let (points = _index_to_points(line, index)) let (
    vectors = [
...
function radius_line_edge(line, index, radius) =
  ...
  let (points = _index_to_points(line, index)) let (point = points[1]) let (
    vectors = [
      points[1] - points[0], // at - before
  ...
  let (angle = point_angle(line, index)) let (p_angle = positive_angle(angle)) let (
    start_angle = atan2(
      cross(vectors[0], vectors[1]),
      vectors[1] * vectors[0]
    )
  ) let (psa = positive_angle(start_angle)) let (sweep_start = sign(angle) * 90 - psa)
  ...
  let (v = vector_as_unit(vectors[0])) let (
    center_offset = [
...
function circle_fragment(radius, start_angle = 0, sweep_by = 360) =
  ...
  assert(
    is_num(sweep_by) && sweep_by != 0 && abs(sweep_by) <= 360
  )

  let (real_start_angle = start_angle % 360) let (end_angle = real_start_angle + sweep_by) let (fragments = get_fragments_from_r(radius)) let (degrees_per_fragment = 360 / fragments * sign(sweep_by))
  //    echo(str("\n",
  ...
  //    ))
  [
      for (i = [real_start_angle:degrees_per_fragment:end_angle]) [cos(i), sin(i)] * radius,
  ];
...
module visualize_line(line, print = false) {
  ...
  size = len(line) - 1;
  new_line = [
    for (i = [size:-1:0]) line[i] + rands(-0.01, 0.01, 2, seed),
  ];
