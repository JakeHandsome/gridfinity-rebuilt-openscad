...
module ClosePoints(pointarrays) {
  ...
  faces_bot = [
    for (i = [0:P - 1]) [0, i + 1, 1 + (i + 1) % len(pointarrays[0])],
  ];
  ...
  faces_loop = [
    for (j = [0:N - 2], i = [0:P - 1], t = [0:1]) [loop_offset, loop_offset, loop_offset] + (
      t == 0 ?
  ...
  faces_top = [
    for (i = [0:P - 1]) [midtop_offset, top_offset + (i + 1) % P, top_offset + i],
  ];

  points = [
    for (i = [-1:NP]) (i < 0) ? midbot
    : (
      (i == NP) ? midtop
...
module ScrewThread(outer_diam, height, pitch = 0, tooth_angle = 30, tolerance = 0.4, tip_height = 0, tooth_height = 0, tip_min_fract = 0) {
  ...
  or =
    (outer_diam_cor < screw_resolution) ?
      screw_resolution / 2
    : outer_diam_cor / 2;
  ...
  pointarrays = [
    for (hs = [0:hsteps]) [
      for (s = [0:steps_per_loop - 1]) let (
        ...
        h_level = (hs % 3 == 2) ? tooth_height - h_fudge
        : (
        ...
        : (1 - hf_w) * wnormal + hf_w * (
          0.1 * screw_resolution + (
            wnormal * wnormal * wnormal / (ext_tip * ext_tip + 0.1 * screw_resolution)
          )
        ),
        r = (ht <= 0) ? ir + w
...
module AugerThread(outer_diam, inner_diam, height, pitch, tooth_angle = 30, tolerance = 0.4, tip_height = 0, tip_min_fract = 0) {
  ...
  ScrewThread(
    outer_diam, height, pitch, tooth_angle, tolerance, tip_height,
    ...
    translate(position)
      rotate(rotation)
        translate([0, 0, -extra_height / 2])
          ScrewThread(
            1.01 * outer_diam + 1.25 * tolerance, height + extra_height,
...
module AugerHole(outer_diam, inner_diam, height, pitch, position = [0, 0, 0], rotation = [0, 0, 0], tooth_angle = 30, tolerance = 0.4) {
  tooth_height = tan(tooth_angle) * (outer_diam - inner_diam);
  ScrewHole(
    outer_diam, height, position, rotation, pitch, tooth_angle,
  ...
  difference() {
    ...
    translate(position)
      rotate(rotation)
        translate([0, 0, -extra_height / 2])
          cylinder(
            h=recessed_height + extra_height / 2,
...
module MetricBolt(diameter, length, tolerance = 0.4) {
  drive_tolerance = pow(3 * tolerance / HexDriveAcrossCorners(diameter), 2) + 0.75 * tolerance;
  ...
  translate([0, 0, diameter - 0.01])
    ScrewThread(
      diameter, length + 0.01, tolerance=tolerance,
...
module MetricCountersunkBolt(diameter, length, tolerance = 0.4) {
  drive_tolerance = pow(3 * tolerance / CountersunkDriveAcrossCorners(diameter), 2) + 0.75 * tolerance;
  ...
  translate([0, 0, diameter / 2 - 0.01])
    ScrewThread(
      diameter, length - diameter / 2 + 0.01, tolerance=tolerance,
      tip_height=ThreadPitch(diameter), tip_min_fract=0.75
    );
}
...
module RodStart(diameter, height, thread_len = 0, thread_diam = 0, thread_pitch = 0) {
  ...
  translate([0, 0, height])
    ScrewThread(
      thread_diam, thread_len, thread_pitch,
...
module RodExtender(diameter, height, thread_len = 0, thread_diam = 0, thread_pitch = 0) {
  ...
  difference() {
    union() {
      ...
      translate([0, 0, height])
        ScrewThread(
          thread_diam, thread_len, pitch=thread_pitch,
          tip_height=thread_pitch, tip_min_fract=0.75
        );
    }
