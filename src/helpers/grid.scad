...
let (rank = len(element_dimensions)) is_undef(perimeter) || (
  is_list(perimeter) && len(perimeter) == 2 * rank
  // None match the condition.
  && min(
    [
      for (i = [0:rank - 1]) perimeter[i] + perimeter[i + rank] < element_dimensions[i] || (
        element_dimensions[i] == 0 && perimeter[i] + perimeter[i + rank] == 0
      ) ? 1
      : 0,
    ]
  ) != 0
...
function new_grid(
  ...
  assert(
    is_list(num_elements) && len(num_elements) >= 2 && min(num_elements) >= 0
  )
  assert(
    is_list(element_dimensions) && len(element_dimensions) == len(num_elements) && min(element_dimensions) >= 0
  )
  assert(is_bool(center))
  assert(
    _is_valid_perimeter(element_dimensions, perimeter),
...
function grid_from_total(
  ...
  assert(
    is_list(total_dimensions) && len(total_dimensions) == len(num_elements) && min(total_dimensions) >= 0
  )
  let (
    element_dimensions = [
      for (d = [0:len(num_elements) - 1]) num_elements[d] == 0 ? 1
...
function grid_get_num_elements(grid) =
  ...
  let (num_elements = grid[1]) num_elements;
...
function grid_get_element_count(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (num_elements = grid[1]) let (rank = len(num_elements)) [
    for(i = 0,a = 1;i < rank;a = a * num_elements[i],i = i + 1)a * num_elements[i],
  ][rank - 1];
...
function grid_get_element_dimensions(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (element_dimensions = grid[2]) element_dimensions;

function grid_is_centered(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (centered = grid[3]) centered;
...
function grid_get_perimeter(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (perimeter = grid[4]) perimeter;
...
function grid_get_average_element_dimensions(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (num_elements = grid[1]) let (rank = len(num_elements)) let (total_dimensions = grid_get_total_dimensions(grid)) [
      for (i = [0:rank - 1]) total_dimensions[i] / (
...
function grid_get_raw_dimensions(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (num_elements = grid[1]) let (element_dimensions = grid[2]) let (rank = len(num_elements)) [
      for (i = [0:rank - 1]) num_elements[i] * element_dimensions[i],
  ];
...
function grid_get_total_dimensions(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (perimeter = grid[4]) let (raw_dimensions = grid_get_raw_dimensions(grid)) let (rank = len(raw_dimensions)) raw_dimensions - [for (i = [0:rank - 1]) perimeter[i] + perimeter[i + rank]];
...
function grid_get_position_bottom_left(grid) =
  assert(is_grid(grid), "Not a grid.")
  let (centered = grid[3]) let (half_raw_dimensions = grid_get_raw_dimensions(grid) / 2) let (rank = len(half_raw_dimensions)) !centered ? [0, 0, 0]
  : -[
...
function is_grid(grid) =
  is_list(grid) && len(grid) == 5 && grid[0] == "grid_struct";
...
function grid_get_element(grid, index, center = false) =
  ...
  assert(
    is_list(index) && len(index) == len(num_elements),
  ...
  assert(
    min(
      [
        for (d = [0:len(index) - 1]) index[d] < num_elements[d] ? 1 : 0,
      ]
    ) == 1,
...
function grid_scale(grid, grid_units = []) =
  ...
  let (size_mm = grid_get_average_element_dimensions(grid)) let (rank = len(size_mm))
...
module grid_translate(grid, index, center = false) {
  ...
  offset = [
    for (i = [0:len(index) - 1]) partial_index[i] * element_dimensions[i],
  ];
...
module grid_foreach(grid, center_elements = false) {
  ...
  for (sequence_number = [0:count - 1]) {
    //Goes x -> y -> z
    index = [
      for(d = 0,i = sequence_number;d < rank;i = floor(i / num_elements[d]),d = d + 1)i % num_elements[d],
    ];
...
module _grid_visualize_perimeter_2d(grid, min_per_side) {
  ...
  assert(
    inner_size.x <= total_mm.x && inner_size.y <= total_mm.y
  );
...
module debug_grid(
  ...
  //Grid Center
  position_center = grid_get_position_center(grid);
  translate(position_center + [0, 0, 3 * layer_spacing]) if (len(position_center) == 2) {
    circle(r=0.75);
  } else {
    sphere(r=0.75);
  }
