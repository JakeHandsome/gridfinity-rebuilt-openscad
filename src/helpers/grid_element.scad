...
function is_grid_element(element) =
  is_list(element) && element[0] == "grid_element_struct" && len(element) == 4;
...
function grid_element_get_grid(element) =
  ...
  let (grid = element[1]) let (index = element[2]) let (centered = element[3]) grid;
...
function grid_element_get_index(element) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (index = element[2]) let (centered = element[3]) index;
...
function grid_element_is_centered(element) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (index = element[2]) let (centered = element[3]) centered;
...
function grid_element_get_position(element) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (index = element[2]) let (centered = element[3])

  let (perimeter = grid_get_perimeter(grid)) let (raw_element_dimensions = grid_get_element_dimensions(grid)) let (element_dimensions = grid_element_get_dimensions(element))
  ...
  ) grid_get_position_bottom_left(grid) + raw_element_offset + perimeter_offset + (centered ? element_offset : [0, 0, 0]);
...
function grid_element_get_sequence_number(element) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (index = element[2]) let (centered = element[3]) let (num_elements = grid_get_num_elements(grid)) let (rank = len(num_elements))

  [
    for(d = 0,a = 0,m = 1;d < rank;a = a + index[d] * m,m = m * num_elements[d],d = d + 1)a + index[d] * m,
  ][rank - 1];
...
function grid_element_get_dimensions(element) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (raw_element_dimensions = grid_get_element_dimensions(grid)) let (perimeter = grid_get_perimeter(grid)) let (rank = len(raw_element_dimensions)) let (
    perimeter_adjustment = [
      for (i = [0:rank - 1]) (grid_element_is_first(element, i) ? perimeter[i] : 0) + (grid_element_is_last(element, i) ? perimeter[i + rank] : 0),
    ]
...
function grid_element_is_last(element, dimension) =
  assert(is_grid_element(element), "Not a grid element.")
  let (grid = element[1]) let (num_elements = grid_get_num_elements(grid)) assert(
    dimension < len(num_elements),
    str("Maximum dimension is ", len(num_elements) - 1)
  )
  let (last = num_elements[dimension] - 1) grid_element_is_in_position(element, dimension, last);
...
module grid_element_label(
  ...
  available_2d =
    rotate_text ? [dimensions.y, dimensions.x]
    : [dimensions.x, dimensions.y];
  final_available = available_2d - [2.5, 2.5];

  text_size = textmetrics(
    output,
    halign="center",
    valign="center"
  ).size;
